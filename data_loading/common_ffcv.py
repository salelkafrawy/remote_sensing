...
#         patches["rgb"]= rgb_patch #torch.Tensor(rgb_patch)
...
#         patches["near_ir"]= near_ir_patch #torch.Tensor(near_ir_patch).unsqueeze(0).unsqueeze(0)

    if "altitude" in data:
        altitude_filename = filename.with_name(filename.stem + "_altitude.tif")
        altitude_patch = tifffile.imread(altitude_filename)
#         patches["altitude"] = altitude_patch # torch.Tensor(altitude_patch).unsqueeze(0).unsqueeze(0)
...
#         patches["landcover"] = landcover_patch #torch.Tensor(landcover_patch).unsqueeze(0).unsqueeze(0).type(torch.LongTensor)
